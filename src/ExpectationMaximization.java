...
public class ExpectationMaximization {
    ...
    private Map<CoinToss, Float> lambdaCountTable;

    //    H T
    //  H
    //  T
    //
    private Float[][] betaCountTable = new Float[2][2];
    ...
    private float denominatorProduct(CoinToss lambdaType, Observation observation){
        ...
        float secondProduct = OppositeLambdaValue * (OppositeBetaFirstToss * OppositeBetaSecondToss * OppositeBetaThirdToss);
    ...
    public void computeEM(CoinToss lambda_type, Parameters param, List<Observation> observations) {
        ...
        while (!done) {
            ...
            if (this.parameters.betaOne == 1 || this.parameters.betaTwo == 1 ||
                    this.parameters.betaOne == 0 || this.parameters.betaTwo == 0) {
                done = true;
            }
    ...
    public float getLogSum(Observation observation) {
        ...
        float term = (float) (((parameters.lambda * Math.pow(parameters.betaOne, frequencyHeads)) *
                                     Math.pow((1 - parameters.betaOne), frequencyTails)) + ((1 - parameters.lambda) *
